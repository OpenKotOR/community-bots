...
8. [REPO] Offline citation gates: **`pnpm trask:gate`** (one build, import smoke, **`trask:config-drift`**, full measure with skip-check, `:ci`; **composite_score** floor **165** on both measure runs). CI runs `pnpm build`, `trask:smoke-imports:ci`, **`trask:config-drift`**, `trask:optimize-measure:ci`, then indexed-stack **`verify:trask-discord:ci`** + **`verify:trask-cli:ci`** (five-query import-smoke, no LLM/token), then `holocron:e2e:playwright`. Live `verify:trask-*` and `holocron:e2e` preflight with `trask:gate`. Optional `pnpm trask:faithfulness-eval` replays fixtures only.
...
- [trask-citation-stack-closeout-2026-05-24.md](../../solutions/tooling-decisions/trask-citation-stack-closeout-2026-05-24.md) — citation arc **PR #33–#74**.
- [trask-citation-module-architecture-2026-05-24.md](../../solutions/tooling-decisions/trask-citation-module-architecture-2026-05-24.md) — module map and gate table (PR #33–#74).
