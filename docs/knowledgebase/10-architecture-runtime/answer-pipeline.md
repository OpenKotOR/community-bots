...
8. [REPO] Offline citation gates: **`pnpm trask:gate`** (build + full `optimize-measure` + `:ci`; **composite_score** floor **165** on both runs). CI runs `pnpm build` then `trask:optimize-measure:ci`. Live `verify:trask-*` and `holocron:e2e` preflight with `trask:gate`. Optional `pnpm trask:faithfulness-eval` replays fixtures only.
