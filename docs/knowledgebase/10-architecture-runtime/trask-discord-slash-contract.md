...
- [REPO] Successful `/ask` embeds set a **one-line footer** via `formatDiscordProvenanceFooter` (e.g. `12 passages · indexer 8787`) from `ResearchWizardBriefAnswer.provenance` (`passagesCount`, `indexerUrl` from retrieve payload). Footer is separate from the ≤5-line description; timeout/synthesis-failure replies keep the default bot footer.
- [REPO] `scripts/verify_trask_discord_live.mjs` asserts provenance footer shape on **`--import-smoke`** (formatter + sample provenance) and on **live** success rows (`result.provenance`, ≥1 passage, http indexer URL).
...
- [trask-citation-display-contract.md](trask-citation-display-contract.md) — citation markers, `formatDiscordAskDisplay` pipeline, verification ladder.
