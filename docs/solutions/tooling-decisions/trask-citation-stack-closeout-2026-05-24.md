...
pnpm trask:verify-import-smoke:ci  # CI: Discord + CLI import-smoke (no token/LLM)
# debug: pnpm verify:trask-cli:ci | pnpm verify:trask-discord:ci
...
## Holocron source cards and GitHub permalinks (2026-06)
...
Holocron Sources panel labels call `webCitationDisplayLabel` via `apps/holocron-web/src/lib/answer-presentation.ts` (import `@openkotor/trask/github-citation-url` — browser-safe subpath). Answer-body markdown sanitization lives in the same module (`sanitizeAnswerParagraph`, peel embedded `Sources` lists).

Related plan: `docs/plans/2026-06-04-007-feat-holocron-answer-citation-render-plan.md`.
