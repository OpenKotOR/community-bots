...
marks = []
try:
    for mark in bm.getBookmarks(addr):
        marks.append({"type": str(mark.getTypeString()), "category": str(mark.getCategory()), "comment": str(mark.getComment())})
except Exception:
    pass
__result__ = {
...
$seqJson = '[' +
  ...
  ']'
$seqPath = Join-Path $tmpDir "verify-k1-amazongames.json"
