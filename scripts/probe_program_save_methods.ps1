...
$code = @'
methods=[]
for m in currentProgram.getClass().getMethods():
    if m.getName() in ["save", "canSave", "isChanged", "isTemporary", "hasTerminatedTransaction", "getCurrentTransaction"]:
        methods.append(str(m))
methods.sort()
__result__={"class": str(currentProgram.getClass()), "methods": methods[:200]}
'@
$seqJson = '[' +
  ...
  ']'
$seqPath = Join-Path $tmpDir "probe-program-save-methods.json"
