...
methods = []
for m in df.getClass().getMethods():
    name = m.getName()
    if "Project" in name or "Parent" in name or "Folder" in name or "File" in name or "Path" in name:
        methods.append(str(m))
methods.sort()
result = {"df_class": str(df.getClass()), "path": str(df.getPathname()), "methods": methods[:200]}
try:
    parent = df.getParent()
    result["parent_class"] = str(parent.getClass())
    pmethods=[]
    for m in parent.getClass().getMethods():
        name=m.getName()
        if "Project" in name or "File" in name or "Folder" in name or "Path" in name:
            pmethods.append(str(m))
    pmethods.sort()
    result["parent_methods"] = pmethods[:200]
...
$seqJson = '[' +
  ...
  ']'
$seqPath = Join-Path $tmpDir "probe-domain-file-methods.json"
