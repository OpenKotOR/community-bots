...
vals={}
for name in ["isReadOnly","inWritableProject","isCheckedOut","canCheckin","modifiedSinceCheckout","isVersioned"]:
    try:
        ...
        vals[name]="ERR:"+str(e)
try:
    vals["pathname"]=df.getPathname()
except Exception as e:
    vals["pathname"]="ERR:"+str(e)
try:
    vals["program_can_save"] = currentProgram.canSave()
except Exception as e:
    vals["program_can_save"]="ERR:"+str(e)
__result__=vals
'@
$seqJson = '[' +
  ...
  ']'
$seqPath = Join-Path $tmpDir "probe-checkout-handle.json"
