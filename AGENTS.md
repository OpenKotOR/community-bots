...
`verify:trask-cli` runs `trask:gate` then `verify_trask_cli_qa.mjs`, which auto-bootstraps indexer+Worker (**8787**/**8790**) when unhealthy (same helper as Holocron e2e). Use when debugging retrieval without the Holocron UI; it does **not** replace `pnpm holocron:e2e`.
