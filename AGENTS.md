...
- **Trask citation modules (PR #33–#88):** Root verify scripts use workspace package imports (`@openkotor/trask`, `trask-config`, `config`, `retrieval`) — see **`docs/solutions/tooling-decisions/trask-root-script-package-imports-2026-05-24.md`**. `citation-markers.ts`, `research-answer-split.ts`, `query-anchor.ts`, `grounded-evidence.ts` (compose), `discord-reply-format.ts` (brief embed). Offline preflight: **`pnpm trask:gate`** (one `pnpm build`, import smoke, **`trask:config-drift`**, full measure with skip-check, then `:ci`); **`pnpm holocron:e2e`**, and **`pnpm verify:trask-discord`** run `trask:gate` before live steps (verify:trask-cli alias present but mjs deleted on branch). CI offline after build: **`pnpm trask:gate:ci`**. Indexed import-smoke (no LLM/token): **`pnpm trask:smoke-imports:ci`** (via gate; Discord+CLI golden via package imports smoke); stack **`pnpm trask:smoke:stack-bootstrap`**. Individual live via `verify:trask-discord` (cli mjs missing). Live QA bootstrap: **`docs/solutions/tooling-decisions/trask-qa-stack-bootstrap-2026-05-24.md`**. **composite_score** floor **165**. After golden/policy edits, gate already includes drift — standalone **`pnpm trask:config-drift`** remains for quick checks. Import smoke also gates AE3 stderr trace failure classes (`scripts/lib/trask_research_trace_assert.mjs`; grep patterns in **`docs/trask-ops.md`**). See **`docs/solutions/tooling-decisions/trask-citation-stack-closeout-2026-05-24.md`** and **`trask-citation-module-architecture-2026-05-24.md`**.
...
Holocron e2e is **functional only** (no mocked `/api/trask` routes, no UI regression fixtures). The primary happy-path spec `apps/holocron-web/e2e/holocron-research.spec.ts` (5 queries) was removed on this branch (c47c52f); only failure-path spec remains and is not wired. Playwright e2e currently discovers 0 tests (see plan 118 residuals). Use Cursor browser MCP + manual :4010 for the 5 canonical as primary gate when available.
...
5. If the task touches deployed/public Holocron behavior, also verify **`https://openkotor.github.io/community-bots/qa-webui/?thread=<fresh-uuid>`** in a real browser after deploy (see **`docs/knowledgebase/30-product-ux/holocron-web-trask-client.md`** and **`docs/knowledgebase/50-execution/validation-ladder.md`** §8). Keep `TRASK_API_BASE` on the Trask worker with **`TRASK_BUILTIN_API=0`** and a live research backend, then confirm at least one public query returns **multiple `https://` sources** before reporting success.
...
`verify:trask-cli` (alias restored in c1f4021) points at missing `verify_trask_cli_qa.mjs` (deleted c47c52f, not restored). Use `pnpm trask:smoke-imports:ci` + gate for CLI golden parity instead; live CLI path covered by import-smoke when stack healthy. `pnpm verify:trask-discord` remains for Discord live.
...
`trask:faithfulness-eval` replays committed golden fixtures under `data/trask-eval/fixtures/` (no live web research). It does **not** replace Holocron e2e for end-to-end research validation. `pnpm holocron:e2e` runs **`pnpm trask:gate`** then `holocron:e2e:playwright` (Playwright `webServer` runs `scripts/holocron-e2e-webserver.mjs`, which bootstraps indexer+Worker when needed and sets `TRASK_QA_GROUNDING=1` for CI-parity keyless compose). `pnpm verify:trask-discord` (cli verify alias present but mjs deleted) also preflight with **`trask:gate`** and auto-bootstrap the indexed stack before live queries. CI runs `pnpm build`, **`pnpm trask:gate:ci`**, indexer+Worker bootstrap, **`pnpm trask:smoke-imports:ci`** (inside gate), then `holocron:e2e:playwright` with `TRASK_SKIP_BUILD=1` (no duplicate full `trask:gate` in CI). Product policy strings live under `data/trask/`; `pnpm trask:gate` includes `trask:config-drift` — use standalone `pnpm trask:config-drift` only for a quick drift-only check after edits.

### Trask Discord `/ask` — mandatory verification (agents)

**Do not claim Discord `/ask` is fixed until live checks pass.** (Note: holocron:e2e and verify:trask-cli currently non-functional due to deleted impl files; use `pnpm verify:trask-discord` + browser MCP for holocron.) 
