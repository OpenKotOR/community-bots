...
- **Trask citation modules (PR #33–#46):** `citation-markers.ts`, `research-answer-split.ts`, `query-anchor.ts`, `grounded-evidence.ts` (compose), `discord-reply-format.ts` (brief embed). Offline gates: `pnpm trask:optimize-measure` (full local) and `pnpm trask:optimize-measure:ci` (CI-equivalent); **composite_score** floor **165**. See **`docs/solutions/tooling-decisions/trask-citation-module-architecture-2026-05-24.md`**.
